...
def newtonraph(x,func,h):
    i = 0
    Y = []
    y=0
    #Loop till x-y reaches required precision
    while i < 200 and abs(x-y) > h:
        i+=1
        y=x
...
def LeastSquare(X,Y):
    ...
    #Creating necessary summation terms
    for i in range(n):
        x1+=X[i]
        y1+=Y[i]
        x2+=pow(X[i],2)
    ...
    return a,b,sigmax,sigmay,covxy,r2
